...
export default function Page() {
  ...
  const items = (data?.jobs || []).filter((j) => j.type === "gather");
  return (
    <div>
      ...
      {items.map((j) => (
        <Link key={j.id} href={`/reports/live/${j.id}`} className="card mb-2 block p-4">
          ...
          <div className="text-sm text-ink-muted">{j.status} · نجاح {j.successCount} · فشل {j.failCount} · {formatDateTime(j.createdAt)}</div>
        </Link>
      ))}
